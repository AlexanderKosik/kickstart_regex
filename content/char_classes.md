...
For these kind of use cases we can define character classes. A character class is defined with square brackets `[]`.

A character classes matches on every element that is contained:

    - [abc] will match on a or b or c
    - [xyz] will match on x or y or z
    - [0123456789] will on 0, 1, ...9

Character classes are kind of an own mini-language within Regular Expressions. Several symbols within character classes have different meanings than outside character classes. This may confuse some beginners when learning and reading existing Regular Expressions written by other people. But this confusing comes mostly from the fact, that this different usage of the symbols is not well known.
...
This looks a lot better than before. But we still have some problems. As you might know, the range of an address segment only ranges from 0 to 255. So all of these examples are actual invalid ip addresses:

    - 192.168.1.256 # invalid address
    - 192.168.1.300 # invalid address
    - 192.168.1.999 # invalid address

But our regex validates everything that is a number. This is a problem. But don't worry - we have a solution for that and we will fix this later :)

## Character classes simplified
What if we want to match all lower case characters. It would not be feasible to create a character class like this: `[abcdefghjklmnopqrstuvwxyz]`. This is very tidious so there must be a better way!

And there is. The dash `-` between `a` and `z` defines a range. So it will match every character: `a`, `b`, `c`, ..., `z`.

Look at the examplpes below and try to predict if they will match or not:
...
So the ranges look cool. Why not use this to fix our problem with invalid IP addresses like `192.168.1.999` in the following manner:
...
Can you figure out, why this does not work? Solution is at the end of the chapter.
