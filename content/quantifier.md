...
Yeeiiih! That's cool! Our RegEx now looks much simpler.

Of course we can also use the quantifier on literal characters. If we want to match the letter `z` exactly 3 times we can do this like this:
...
Let's have a closer look on some other quantifiers.
...
In the example above we could extract the IP adresses also manually. Our test content has only 7 entries. But in a real world scenario the log file may have more than 10.000 lines. A manual extraction would be very dedious to say at least.
...
Let's say we want to match opening HTML-Tags `<` starting with a `h`. We do not want the closing ones, so we would only expect: `<html>, <head>, <h1> and <h3>`.
...
# we now want to find every occurence of <html-tags> starting with a lower case `h`.
...
Solution is at the end of the file.
...
*Please be aware that validating an email is a lot more complex in real world. This is just a simplfied version. So do not use this validator in production code! ;)*
